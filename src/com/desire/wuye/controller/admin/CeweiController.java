package com.desire.wuye.controller.admin;

import com.desire.wuye.controller.BaseController;
import com.desire.wuye.controller.BaseInterface;
import com.desire.wuye.model.Cewei;
import com.jfinal.ext.route.ControllerBind;

/**
 * Created by Administrator on 14-4-13.
 */
@ControllerBind(controllerKey = "/admin/cewei")
public class CeweiController extends BaseController implements BaseInterface {

    @Override
    public void add() {
    }

    @Override
    public void delete() {
        Cewei.dao.deleteById(getPara());
        setAttr("msg", "删除成功！");
        renderJson();
    }

    @Override
    public void create() {
        getModel(Cewei.class).save();
        setAttr("msg","保存成功！");
        renderJson();
    }

    @Override
    public void edit() {
        setAttr("cewei",Cewei.dao.findById(getPara()));
    }

    @Override
    public void update() {
        getModel(Cewei.class).update();
        setAttr("msg", "保存成功！");
        renderJson();
    }

    @Override
    public void index() {
        String state=getPara("state");
        if(state==null){
            state="";
        }
        int a=1;
        if(getParaToInt()!=null)
            a=getParaToInt();
        String query="%"+getPara("query")+"%";
        System.out.println(state);
        if(!query.equals("%null%")){
            setAttr("cewei", Cewei.dao.paginate(a,PAGE_SIZE,"select * ","from cewei where ( zhuhu_no like ? or name like ? or no like ?) and  kong like '%"+state+"%'"
                    ,query
                    ,query
                    ,query));
        }
        else {setAttr("cewei", Cewei.dao.paginate(a,PAGE_SIZE,"select * ","from cewei where kong like '%"+state+"%'"));
        }
        if(getPara("print")!=null){
            render("print.html");
        }
    }

    @Override
    public void view() {
        setAttr("cewei",Cewei.dao.findById(getPara()));
    }

    @Override
    public void change() {

    }
}
